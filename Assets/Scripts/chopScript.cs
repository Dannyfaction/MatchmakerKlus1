...
public class chopScript : MonoBehaviour
{
    ...
    public static void removeStuffFromChild(GameObject part)
    {
        ...
        if (part.GetComponent<Collider>()) GameObject.Destroy(part.GetComponent<Collider>());
        if (part.GetComponent<Rigidbody>()) GameObject.Destroy(part.GetComponent<Rigidbody>());
    }
